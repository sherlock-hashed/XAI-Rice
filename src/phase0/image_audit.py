...
def inspect_single_image(
    ...
    Does NOT transform or modify the image on disk.
    """
    path = Path(file_path)
    ...
    if compute_hash:
        try:
            record["sha256"] = compute_file_sha256(str(path))
        except Exception as e:
            record["anomalies"].append(f"hash_error: {str(e)}")

    # Non-destructive header & content readability test with Pillow
    try:
        with Image.open(str(path)) as img:
            img.verify() # First verify integrity of file structure
        
        # Re-open to read properties (since verify closes or invalidates some fields)
        with Image.open(str(path)) as img:
...
def audit_images(
    ...
    for rel_p in image_relative_paths:
        ...
        rec = inspect_single_image(str(full_path), str(root), compute_hash=compute_hashes)
        
        # Determine class label
        if class_extractor:
            class_name = class_extractor(rel_p)
        else:
            # Default heuristic: immediate parent folder
    ...
    # Imbalance metric: max_class / min_class
